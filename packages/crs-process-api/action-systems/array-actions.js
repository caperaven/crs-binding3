class ArrayActions {
  ...
  static async transfer(step, context, process, item) {
    ...
    const target = await crs.process.getValue(step.args.target, context, process, item);
    const value = await crs.process.getValue(step.args.value, context, process, item);
    await this.add({ args: { target, value } }, context, process, item);
    await this.remove({ args: { target: source, value } }, context, process, item);
  }
  ...
  static async map_objects(step, context, process, item) {
    ...
    for (const item2 of collection) {
      const res = await Promise.all(fields.map((f) => crsbinding.utils.getValueOnPath(item2, f)));
      result.push(...res);
    }
    if (step.args.target != null) {
      await crs.process.setValue(step.args.target, result, context, process, item);
