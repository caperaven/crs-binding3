...
var SchemaRegistry = class {
  constructor() {
    this._schemas = {};
    (crs.binding || crs.binding).events.emitter.on("run-process", this._runProcess.bind(this));
  }
  _runProcess(args) {
...
var ProcessRunner = class {
  ...
  static async cleanProcess(process) {
    if (process.bindable == true) {
      crs.binding.data.removeObject(process.parameters.bId);
    }
...
async function initialize(root) {
  ...
  await crs.modules.add("fixed_position", `${root}/action-systems/fixed-position-actions.js`);
  ...
  await crs.modules.add("scripts", `${root}/action-systems/scripts-actions.js`);
  ...
  await crs.modules.add("schema", `${root}/action-systems/schema-actions.js`);
